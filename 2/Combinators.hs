...
p <|> q = \inp ->
  let (_, inp') = span isWhiteSpace inp in
  case p inp' of
    Error _ -> q inp'
    result  -> result
...
span' :: (Char -> Bool) -> Parser [Char]
span' pred inp =
  case elem inp of
    Success (c, cs) | pred c -> Success (span pred inp)
                    | otherwise -> Error "Predicate is not satisfied"
    Error err -> Error err
